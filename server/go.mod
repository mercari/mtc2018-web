module github.com/mercari/mtc2018-web/server

require (
	github.com/99designs/gqlgen v0.4.4
	github.com/DataDog/datadog-go v0.0.0-20180822151419-281ae9f2d895 // indirect
	github.com/DataDog/opencensus-go-exporter-datadog v0.0.0-20180731092159-dbb3980bd763
	github.com/agnivade/levenshtein v1.0.0 // indirect
	github.com/andreyvit/diff v0.0.0-20170406064948-c7f18ee00883 // indirect
	github.com/davecgh/go-spew v1.1.1 // indirect
	github.com/inconshreveable/mousetrap v1.0.0 // indirect
	github.com/kelseyhightower/envconfig v1.3.0
	github.com/opentracing/opentracing-go v1.0.2 // indirect
	github.com/philhofer/fwd v1.0.0 // indirect
	github.com/pkg/errors v0.8.0
	github.com/pmezard/go-difflib v1.0.0 // indirect
	github.com/sergi/go-diff v1.0.0 // indirect
	github.com/spf13/cobra v0.0.3 // indirect
	github.com/spf13/pflag v1.0.2 // indirect
	github.com/stretchr/testify v1.2.2
	github.com/tinylib/msgp v1.0.2 // indirect
	github.com/vektah/gqlparser v0.0.0-20180823070515-d0fbd67c7fe9 // indirect
	go.opencensus.io v0.15.0
	go.uber.org/atomic v1.3.2 // indirect
	go.uber.org/multierr v1.1.0 // indirect
	go.uber.org/zap v1.9.1
	golang.org/x/lint v0.0.0-20180702182130-06c8688daad7
	golang.org/x/net v0.0.0-20180826012351-8a410e7b638d // indirect
	golang.org/x/sys v0.0.0-20180828065106-d99a578cf41b // indirect
	golang.org/x/text v0.3.0 // indirect
	golang.org/x/tools v0.0.0-20180828015842-6cd1fcedba52 // indirect
	gopkg.in/DataDog/dd-trace-go.v1 v1.2.2
	gopkg.in/yaml.v2 v2.2.1 // indirect
)
